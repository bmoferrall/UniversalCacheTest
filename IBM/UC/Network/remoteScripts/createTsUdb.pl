...
sub main {
	...
	if ( $cdcRootDir && $db2Dir && $tsName && $tsPort &&  $dbName && $db2User 
			&& $db2Pass	&& $dbPort && $dbSchema && $cdcRefreshLoader 
			&& $cdcbits && $remoteCdcSolidRootDir)
	{
		...
		my $db2Db = Db2DB->createFromConfig(DatabaseConfig->createFromHash($db2Cfg));
		my $cdcCfg = { 
			ts_name => $tsName, 
			ts_port => $tsPort,
			ts_root => $cdcRootDir,
		...
		my $cdcUdb = CDCDb2->createFromConfig(CDCConfig->createFromHash($cdcCfg), $db2Db);
		$cdcUdb->create();
	}
}
main();