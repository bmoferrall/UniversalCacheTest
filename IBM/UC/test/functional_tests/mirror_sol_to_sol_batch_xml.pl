...
print "*********************************************\n";
print "THIS ROUTINE PERFORMS A MIRROR FROM A SOLID\n";
print "SOURCE TO A SOLID TARGET FOR A BATCH OF 100\n";
...
eval { # trap errors so we can clean up before exiting
	...
	$dbSrcCfg = DatabaseConfig->new('sol_to_sol.ini');
	...
	$dbTgtCfg = DatabaseConfig->new('sol_to_sol.ini');
	...
	for (my $i=1; $i<=99; $i++) {
		my $name = 'DBA.SRC' . $i;
		my $namet = 'DBA.TGT' . $i;
		$dbSrc->execSqlStmt("create table $name (id integer, name char(50))");
		$dbTgt->execSqlStmt("create table $namet (id integer, name char(50))");
	}

	# create source CDC
	$cdcSrcCfg = CDCConfig->new('sol_to_sol.ini');
	$cdcSrcCfg->initFromConfigFile("\\[CDC_SRC\\]","\\[/CDC_SRC\\]");
	$cdcSrc = CDCSolid->createFromConfig($cdcSrcCfg,$dbSrc);

	# create target CDC
	$cdcTgtCfg = CDCConfig->new('sol_to_sol.ini');
	...
	$asCfg = AccessServerConfig->new('sol_to_sol.ini');
	...
	$mConsole->addMappingsFromFile(Environment->getEnvironment()->getStartDir().'tablemappings.xml', $sub); 
	...
	for (my $i=1; $i<=99; $i++) {
		my $name = 'DBA.SRC' . $i;
		$dbSrc->execSqlStmt("insert into $name values (1,'One')");
		$dbSrc->execSqlStmt("insert into $name values (2,'Two')");
		$dbSrc->execSqlStmt("insert into $name values (3,'Three')");
	}
	...
	for (my $i=1; $i<=99; $i++) {
		my $namet = 'DBA.TGT' . $i;
		$dbTgt->execSqlStmt("select * from $namet");
	}
