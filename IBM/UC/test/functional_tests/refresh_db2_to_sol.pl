...
print "*********************************************\n";
print "THIS ROUTINE PERFORMS A REFRESH FROM A DB2\n";
print "SOURCE TO A SOLID TARGET\n";
print "*********************************************\n\n\n";
sleep(3);
refreshSolidToDB2();
...
eval { # trap errors so we can clean up before exiting
	...
	# create source CDC
	$cdcSrcCfg = CDCConfig->new('db2_to_sol.ini');
	$cdcSrcCfg->initFromConfigFile("\\[CDC_SRC\\]","\\[/CDC_SRC\\]");
	$cdcSrc = CDCDb2->createFromConfig($cdcSrcCfg,$dbSrc);

	# create target CDC
	$cdcTgtCfg = CDCConfig->new('db2_to_sol.ini');
	...
	$asCfg = AccessServerConfig->new('db2_to_sol.ini');
	...
	# Mark source table for refresh
	$mConsole->flagForRefresh($dsSrc,$tblNameSrc,$sub);
	...
	$mConsole->refreshSubscription($dsSrc,$sub);
